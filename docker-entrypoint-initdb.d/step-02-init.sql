-- Ensure target database exists (CREATE DATABASE cannot run in a transaction, so use psql's \gexec)
SELECT 'CREATE DATABASE films' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'films')\gexec
